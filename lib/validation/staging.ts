...
export const PatchStagingSchema = CreateStagingSchema.partial().extend({
  ...
  // FD3-1 (fix round 4): round 3's OPTIONAL `ifUnitCostCents` precondition is
  // GONE from this surface. It existed for the freight panel's per-line fan-out,
  // and the fan-out itself was the defect (three rounds of partial-commit
  // hazards) — a bill is now one atomic request to
  // POST /api/inbound-shipments/[id]/costs, which owns the paired rule. What is
  // left here is the MANUAL per-line cost save, unconditional as originally
  // built.
});
