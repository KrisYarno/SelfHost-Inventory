...
export async function savePlatformCredentialsExample(
  ...
  const _encryptedApiKey = encryptValue(credentials.apiKey);
  const _encryptedApiSecret = encryptValue(credentials.apiSecret);
  const _encryptedWebhookSecret = encryptValue(credentials.webhookSecret);
 ...
 */
export async function fetchPlatformOrdersExample(_platformConfigId: number) {
...
export async function syncOrdersFromAllPlatformsExample() {
  ...
  for (const config of configs) {
    ...
    try {
      ...
      const _orders = await fetchPlatformOrdersExample(config.id);
 ...
 */
export async function generateTestWebhookExample() {
