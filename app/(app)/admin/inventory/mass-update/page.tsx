...
import { Save, RotateCcw, AlertTriangle, Download, Search, Shield, RefreshCw } from "lucide-react";
import { toast } from "sonner";
import { cn } from "@/lib/utils";
import { useCSRF, withCSRFHeaders } from "@/hooks/use-csrf";
import { BatchUpdateResult, FailedUpdate, MassUpdateChange } from "@/types/mass-update-errors";
...
export default function AdminMassInventoryUpdatePage() {
    ...
    if (mounted) {
      ...
      if (recoveryManager.hasRecoverableFailures()) {
        ...
        if (state) {
          ...
          toast.warning(
            `${state.failedUpdates.length} updates from your last session failed. You can retry them.`,
            {
              duration: 7000,
              action: {
                label: "Review",
  ...
  const handleQuantityChange = (productId: number, locationId: number, value: string) => {
    ...
    setData((prevData) => {
      ...
      const totalChanges = updatedProducts.reduce(
        (sum, product) => sum + product.locations.filter((loc) => loc.hasChanged).length,
        0
      );
  ...
  const handleSaveAll = async (isRetry = false, retryChanges?: MassUpdateChange[]) => {
    ...
    try {
      // Collect all changes
      const changes: MassUpdateChange[] = isRetry
        ? retryChanges!
        ...
        if (!isRetry) {
          ...
          setData((prevData) => {
            ...
            return {
              ...
              products: prevData.products.map((product) => ({
                ...
                locations: product.locations.map((location) => {
                  ...
                  return {
                    ...location,
                    currentQuantity:
                      wasSuccessful && location.newQuantity !== null
              ...
              totalChanges: prevData.products.reduce(
                (sum, product) => sum + product.locations.filter((loc) => loc.hasChanged).length,
                0
              ),
            };
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold">Mass Inventory Update</h1>
          <p className="text-muted-foreground mt-1">
            Enter physical count numbers to update inventory across all locations. Supports updating
            100+ products efficiently.
          </p>
        </div>
      ...
      {lastFailures.length > 0 && !showRecoveryDialog && (
        <Alert className="border-yellow-600 bg-yellow-50 dark:bg-yellow-900/20">
          ...
          <AlertDescription className="flex items-center justify-between">
            <span>
              {lastFailures.length} update{lastFailures.length !== 1 ? "s" : ""} from a previous
              attempt can be retried.
            </span>
            <Button variant="outline" size="sm" onClick={() => setShowRecoveryDialog(true)}>
              Review Failures
            </Button>
          </AlertDescription>
        </Alert>
        ...
        <CardHeader>
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              ...
              <CardDescription>
                Enter the actual counted quantities. The system will calculate adjustments
                automatically.
              </CardDescription>
            </div>
            <div className="flex gap-2">
                      ...
                      <div className="flex flex-col items-end gap-1">
                        <Input
                          ...
                          className={cn(
                            ...
                            location.delta > 0 && "text-positive",
                            location.delta < 0 && "text-negative"
                          )}
                          placeholder="--"
                          disabled={isSaving}
                        />
