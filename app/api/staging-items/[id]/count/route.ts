...
export const POST = apiHandler(async (request: NextRequest, { params }: RouteParams) => {
  ...
  // FD6-1 (fix round 8) — THE HOUSE DEADLOCK RETRY, ON THIS SIDE OF THE CYCLE TOO.
  //
  // This transaction holds its item and then asks for that item's shipment; the
  // freight bill (POST /api/inbound-shipments/[id]/costs) holds the header and
  // then asks — through its `FOR UPDATE` membership read — for lines it does not
  // hold. That is a genuine cycle, and InnoDB resolves it by rolling back the
  // transaction that did the LEAST work: this one, essentially always. The bill
  // was already wrapped and would return its named 409; an unwrapped count died
  // with an unhandled 500 for a race it did not cause.
  //
  // Re-running is safe because nothing this transaction decides is derived
  // outside it: the before-image read, the claim, the shipment guard, the
  // exception row and the audit event are all re-derived from a fresh snapshot,
  // and a rolled-back attempt left none of them behind. `countedAt` is
  // deliberately NOT regenerated (graduate.ts's batchId rule) — it is the instant
  // the count was taken, not the instant a retry happened to succeed.
