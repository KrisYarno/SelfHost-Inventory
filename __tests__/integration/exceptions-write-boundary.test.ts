...
const ALLOWED_WRITER_CALLERS: WriterCaller[] = [
  ...
  },
  {
    ...
    reason:
      ...
      "row — `recount-corrected`, note 'line removed' when the settlement is new — inside the " +
      "discard's own transaction, and ZERO its money with it (fix-delta 2 FD2-1, removed-line " +
      'money), because a line that left the order has neither a shortage nor a surplus. A no-op ' +
      'for an ORDERED line, which never had a row',
  },
  {
